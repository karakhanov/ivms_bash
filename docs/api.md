...
**Назначение**: создание нового сотрудника через API (форма в админ‑панели фронтенда).

**Тело (JSON)** — основные поля (без вычисляемых):
...
- Фото (`photo`) отправляется **отдельно** как `multipart/form-data` (FormData на фронте), вместе с остальными полями.

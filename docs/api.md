...
1. **Получить токен**: `POST /api/auth/token/`
   - Тело (JSON): `{"username": "<логин>", "password": "<пароль>"}`
   - Ответ 200: `{"token": "<строка-токен>"}`
   - Пользователь должен существовать в Django (админка → Users; или `createsuperuser`).
   - Токен создаётся автоматически при первом успешном запросе (или вручную в разделе Admin → Auth Token).

2. **Использовать токен** в запросах к API:
   - Заголовок: `Authorization: Token <строка-токен>`
   - Пример: `Authorization: Token 9944b09199c62bcf9418ad846dd0e4bbdfc6ee4b`

3. **Без токена** (или с неверным): ответ **401 Unauthorized**.

4. **Вебхук терминалов** (`POST /api/ivms/events/`, `POST /api/ivms/webhook/`) **не требует** авторизации — терминалы не передают заголовок; доступ при необходимости ограничивают файрволом или отдельным секретом в заголовке.
